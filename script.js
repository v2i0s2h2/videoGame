...
🌟 APP: Fighting Game
 ...
 first you need to think , which type of fighting game you want to make, let's i share what you can thing

there will be two player
both have two behavior 
1 - attack
2 - heal
...
reather than making two player,make a class(template) of player, 
----------------------------------------------------------
let discuss about game, i mean 
-when you stop fighting or game
-how you declare winner
-how you reset the game to play again
-and what about autoplay
these are the four behavior i want in this game 

so we need many functions to get these behavior
instead making different function let make methods inside a class
like this
class Game {
 - checkIsOver()
 - declareWinner()
...
----------------------------------------------------------------
finally we need to update dom in every attack and heal and in autoplay
...
const updateGame = (p1, p2, gameState) => {
  ...
  if (p1.health <= 0 || p2.health <= 0) {
    ...
    resultDiv.innerText = game.declareWinner(this.isover,
      p1, p2)
    // we can't access methods directly so we use game.decl...

    return gameState
    
  }
}
// ** Create the Player class w
...
class Game {
  ...
  reset(p1, p2) {
    ...
    // this.isover as a argument is required to reset, because it will update this.isover is false
...
let player1 = new Player("kallu", 100, 10)

let player2 = new Player("ballu", 100, 10)
...
// ** Add a click listener to the simulate button that runs the play() method on click and pass in the player
playButton.onclick = () =>
...
// Add functionality where players can press a button to attack OR health
